...
from funcs import *  # Importe das funções do módulo "funcs".

control = True  # Variável de controle de execução do programa (loop while da linha seguinte).

while control:
    num = num_inputer()  # Variável que recebe o número a ser analisado.
    alg_len = lenght_counter(num)  # Variável que contém o número de algarismos que o número contém para as decisões.

    if num == 0:  # Caso o número seja literalmente zero...
        print('Zero\n')
    else:
        if alg_len == 3:  # Caso o número tenha três algarismos.
            print(three_algarisms_namer(num) + '\n')
        elif alg_len == 2:  # Caso o número tenha dois algarismos.
            print(two_algarisms_namer(num) + '\n')
        elif alg_len == 1:  # Caso o número tenha um algarismo.
            print(one_algarism_namer(num) + '\n')
